// ใช้ IIFE + DOMContentLoaded เผื่อบางโฮสต์ไม่รองรับ defer
(function () {
  const onReady = () => {
    // ====== DOM ======
    ...
    const safeListen = (el, events, handler) => {
      if (!el) return;
      events.forEach(evt => el.addEventListener(evt, handler, { passive: false }));
    };
    safeListen(divQuot, ['mousedown','touchstart','pointerdown'], e => e.stopPropagation());
    safeListen(answerInput, ['mousedown','touchstart','pointerdown'], e => e.stopPropagation());
    ...
    function render(){
      ...
      tiles.forEach(t=>{
        ...
        const startDrag = (e)=>{
          // ถ้าแตะ/คลิกบน input ให้ปล่อยให้ input ทำงาน
    ...
    // ====== Formatting helpers (ใช้กับพหุนาม) ======
    const fmtTerm = {
      x2(k, isLeadDeg2){
    ...
    function newExample(){
      ...
      if(mode==='int_add'){
        ...
        // จำกัดทุกตัวเลขที่แสดงในช่วง [-15,15] และไม่เอา 0 (ตัวตั้ง, ตัวหาร, ผลลัพธ์)
        let divisor, q, dividend;
        do {
          divisor = randNZ();      // [-15,15]\{0}
          q = randNZ();            // [-15,15]\{0}
          dividend = divisor * q;  // ตัวตั้ง
        } while (Math.abs(dividend) > 15); // ต้องไม่เกินช่วงด้วย

        problemText = `${dividend} ÷ ${divisor<0?`(${divisor})`:divisor}`;
        answerCoef={a2:0,a1:0,a0:q}; problemAnswer=String(q);

        showWorkspace('div');
        if (divDivisor) divDivisor.textContent = divisor;
        ...
        if(mode==='poly_add'){
          ...
          problemAnswer = coefToHTML(answerCoef);
          showWorkspace('mul');
          if (mulMult)  mulMult.innerHTML  = coefToHTML(Q.coef).replace(/\+ \(-/g,'+ (-');
          if (mulMcand) mulMcand.innerHTML = coefToHTML(P.coef).replace(/\+ \(-/g,'+ (-');
        }
      }else if(mode==='poly_div'){
        // บังคับตัวหารเป็น (x + d) และทำให้สัมประสิทธิ์ที่ "แสดง" ไม่เกิน 15
        let a,b,d,ok=false;
        while(!ok){
        ...
        const A1v = a*d + b;
        const A0v = b*d;
        const termA1 = `${A1v>=0? '':'('}${A1v>=0?A1v:(A1v+')')}x`;
        const termA0 = `${A0v>=0? '':'('}${A0v>=0?A0v:(A0v+')')}`;
        const dividendHTML = `[${aLead}x<sup>2</sup> + ${termA1} + ${termA0}]`.replace(/\+ -/g,'+ (-');
        ...
        showWorkspace('div');
        if (divDivisor) divDivisor.innerHTML = `x${d>=0?'+':''}${d}`;
        if (divQuot) divQuot.value='';
      }else if(mode==='solve_lin'){
        // สุ่มให้ค่าที่ "แสดง" อยู่ในช่วง [-15,15] และไม่เป็น 0 (รวม d)
        let a,b,c,x,d,ok=false;
        while(!ok){
          a=randNZ(); b=randNZ(); c=randNZ();
          x = rint(-15,15); if(x===0) continue;
          d = a*x + b - c*x; // ต้องอยู่ในช่วงด้วย
          if (Math.abs(d)<=15) ok=true;
        }
        const aLead = (a===1?'':a===-1?'-':'');
        const cLead = (c===1?'':c===-1?'-':'');
        problemText = `${aLead}x${b>=0?'+':''}${b} = ${cLead}x${d>=0?'+':''}${d}`.replace(/\+\-/g,'-');
    ...
    if (modeSel) modeSel.onchange = (e)=>{ mode = e.target.value; newExample(); };
    const newBtn = document.getElementById('btn-new');
    if (newBtn) newBtn.onclick = ()=> newExample();
    const solBtn = document.getElementById('btn-solution');
    if (solBtn) solBtn.onclick = (e)=>{
      showSol = !showSol;
